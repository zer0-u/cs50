...
- hash関数の戻り値が最小でも97(aのASCIICODE)なのでインデックスの外にある
  - それでも例外にならないの困る
- 何かの課題でやった通り、aとの差分を取ってインデックスの代わりにする
- 後で直すところだから適当に済ませておく
- tableが想定通りの構造を持つことは確かめられた

### pset size関数
- 辞書が持つ単語数を返す
- size関数の中で数えてもいいし、読み込む際に一工夫入れてもよい
- ここも工夫すべき箇所らしく説明がほとんどなかった
- グローバル変数word_countを宣言し、load関数の中で数えることにした
- 一応動いたっぽい？
- dictionaries/smallだとちょっとサイズが小さすぎる
- mediumを作ってみよう
  - 50個程度・自由に変更可
- きちんと数えられていることが分かったのでsize関数は完成したとみなす

### pset check関数
- 単語が辞書の中にあればtrue、なければfalse
  - 大文字・小文字は区別しない
- 動画内ではstrcasecmpを使うよう言われているけれど、ハッシュ計算時に小文字化しているのでstrcmpでも行ける気がする
  - 辞書内の構造が謎なので念のためstrcasecmpとしておく
  - と思ったけど小文字化はhash関数の中でやっているのでcheck関数には関係なかった
  - strcasecmpを使おう
- 連結リストのループ処理はlist_4.cからコピーしてきた
- 恐らく動いている
...
- 変数cursorとtempを利用する方法
  ...
  - 4 tempが指すnodeを解放する
  - 5 手順2に戻る
  - list_4.cに書いてある手順と同じだった
- hash関数等と違って手法が説明されているってことは、これが最速なのかなー
- ひとまずこれで実装しよう
- できた
  - ようやく処理時間まで表示されるようになった
...
- 仕方がないのでcheck50を実行
  - ぽろぽろミスがある
  - strcmpを使ったままだった
  - valgrindは同じようなエラーが出ている
- strcasecmpにしたことでロジックの間違いはなくなった

### valgrind問題
- dictionaryに問題があるのか、コマンドそのものに問題があるのか
- これまでちゃんと実行できたプログラムをやり直してみる
- lecture/tree.cでは問題なく実行できた
- 試しにunload関数の中身をすべてコメントアウトした＝意図的にメモリを解放しないまま実行してみた
  - 同じエラーが出る
- 何も実装していない状態だとどうなるんだろう

### valgrind問題(指示通り)
- 演習ページの指示通りpset5フォルダを作ってみる
- 相変わらずNは#defineにしないとコンパイルできない
...
- 最新版の演習ページを見つけた
  ...
  - 最新版を落としてきたらなぜかうまく動いている
  - defineの置き換えもいらない

### pset アップデート
- ざっと見た限り大幅な変更はないと思うので、これまで書いてきたプログラムを2023版に移す
- これまでの内容は2019spellerとして保存
- 移植したら実行もvalgrindも問題なくできた
  - なんだったんだろう？
- 模範解答は ./speller50 で確認できる
  - checkにかかる時間がかなり短くなっていた
  - speller50.txtとspeller.txtに結果を保存している
- check50も通った
- 何が違うのかなーと軽く探ったところ、Makefileのclangコマンドに-gdwarf-4がついている
  - ここに付けるオプションだったのか～
- ともかく完成したのでこれにて良しとする！