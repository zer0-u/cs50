...
- 先頭文字列を判断するのに正規表現を使えるかもしれない？
  - やっぱり面倒だからやめる
  - カード会社のパターンが増えた時を考えると愚直にifで分岐した方がわかりやすいと思う
- チェックサムの計算がやはり面倒
- 結局C言語版を見て移植している
  - 当時の自分がかなり頑張っていることが分かった
- is_evenはかなりこじつけの変数名なのでどうにかしたい
  - C言語と同じように反転させる？
- 文字列の反転は[::-1]でできるらしい
- 変に凝ったやり方を使わない方が分かりやすい気がする
- なんかうまく行かない
  - 反転はできている
- pythonのdebug50ってできないのかな
  - 普通にできた
- 途中で小数になっている
  - /ではなく//を使うべきだった
  - 型に慣れているとうっかりハマる
- とりあえず問題文にある例は全部通った
- check50も通ったのでよしとする
- 文法上の問題があると「アウトライン」に赤い！が出るがエディタには反映されない
  - 連動してくれ～！
...
- これはpython独自っぽい
- そもそもの仕様が難しい……
  - sequencesフォルダにあるそれぞれのDNA配列が誰に該当するかを探せばいいんだろうか
- DNAは特徴的なパターン(STR)が繰り返される部分がある
  - 繰り返される回数は人によって異なる
...
#### sequence読み込み
- next(reader)は配列を返す
- 1行しかないのでnext(reader)[0]でファイルの内容が取り出せる
- file.read()でも取り出せることが分かったが最後に不要な改行が含まれる
  - 取れないのかなー
  - readメソッドがファイル全体の内容を読み込んでいるとしたら妥当な動きではある
  - rstripで取れそう→取れた

#### STR出現回数のチェック
- まずSTRの具体的な文字列を知らなければならない
- 文字列[i:j]でi文字目からj-1文字目までを切り出せる
- STRの具体的な文字列を取る方法は分からない
  - とりあえずベタ打ちで数える部分を実装してみる？
- なんか変だなと思ったら[i:j]とすべきところを[i::j]としていた
  - 見間違い怖い
- 繰り返しの最大回数なんだよな～どうしたらいいんだろう
  - 繰り返し回数を覚える変数とは別に「最大回数」を表す変数を持っておく
- for i in range()はループの中でiをいじれない
  - 戻ってしまう

## 0305

### pset DNA
- 昨日は途中で突然codespacesが起動しなくなって中断してしまった
- なんだったんだろう？

#### STR出現回数のチェック
- 無理にforを使わずwhileでやったほうがよさそう
- 1文字目と合致するかの分岐も要らない
  - あった方が処理コストは下がるかもしれないが、今は行数を減らす方を優先したい
- 細かなtypoが多い
  - 気をしっかり持とう
- 恐らく昨日試した感じの結果と合致するものが得られた

#### database照合
- 数えた結果を何らかのデータ構造(dict?)に保存する
- 回数を比べる時はint()で整数型に変換する
- STRの種類が違うことについて動画では特に何も言われなかった
- dictのkey名を取る方法ってあるのかな
- for 変数 in dictオブジェクト としたらkey名だけが出てきた
  - ちょっとよくわからないので小さなサンプルで実験したい

#### dicttest
- dictオブジェクトからkeyだけを取り出したい
- keys()メソッドを呼び出すとdict_keys型のオブジェクトが返ってくる
- 戻り値を使ってlist関数を呼び出すとlistにできる
- 頭の要素(name)は不要だからlistをスライスする
  - リスト[1:] でOK
- これを使って「参照したいSTRのlist」が作れるはず

#### database照合
- dictのkey名に変数が使えるメリットを初めて活用できた
- 型が違うと==で検証できないことが分かった
  - やっぱりint()は必要
- 一通り実装できた気がする
...
- 再びテスト
  - 大丈夫そう
  - むしろなぜcountをリセットしていないバージョンでも1～4が大丈夫だったのか謎