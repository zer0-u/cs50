...
- 引き分けの存在が謎
- 試しに「引き分けの時は両方のペアを追加する」としてみる
    - 変わらなかった
- そもそもなぜこの実装が必要なのかが分からないので引き分けのケースも考えが深められない
- 引き分けの場合はペアを配列に追加しないが、変数pair_countには引き分けの場合も含めたペア数を代入する必要がある？
- 引き分けだった時にbreakではなくcontinueを使うようにしたら何もかもダメになった
    - 戻したいけどどうなってたか思い出せない
- 直ったしcontinueでよかったらしい
    - pair_countを使うべきところでcandidate_countを参照していた
    - breakだと引き分け以降のチェックがされないのが問題だったのかもしれない
- 今日はここまで
- マージソートとかそういう速い方法を使っていたらcheck50も早くなるのかな～というのは少し気になる
    - ネットワーク通信のコストの方が大きすぎて実装の差は誤差程度かもしれない

## 1225
- すべての記憶を失ってしまった
    - やはり課題は一気に仕上げたほうがよい
- とりあえずもう一度check50をしてみる
    - lock_pairが未完成

### lock_pair(修正)
- 結局Qiitaの記事を参考にしてしまった
    - 何をしているのか完璧に説明しきれない
    - 開始・終了のペアを取って、開始地点をどんどん遡る→終了地点に繋がるものがあればNGみたいな理解
